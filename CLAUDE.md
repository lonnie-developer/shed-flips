...
- **Landing page** at `/<slug>` — generated by the dynamic route at `src/pages/[property].astro`. Hero shows the channel avatar, name, tagline, description, and an "All channels" link back to the homepage; below that is a filtered feed of that property's posts. Slugs: `shed-flips`, `im-gonna-pack-it`, `garage-flips`, `podcast`. **No Subscribe-on-YouTube button** in the hero — see § *Generic YouTube CTAs are intentionally absent* below.
- **Accent color** that drives `--accent` and `--accent-dark` site-wide when you're on that property's landing page or one of its posts (set via inline style on `<html>`). Cards on the homepage and `/blog` mixed feed pick up the property's accent on the chip, the thumbnail-area gradient, and the colored top-bar of the card even when the surrounding page is on a different accent.
- **Property chip** rendered on every PostCard (top-left of the thumbnail). The PostCard component (`src/components/PostCard.astro`) is also where the per-card colored top-bar lives.
- **Channel avatar** in two places: `src/assets/brand/<property>-logo.jpg` (large hero on the landing page + 36×36 in the header nav button) and `public/avatars/<slug>.jpg` (200×200 thumbnail used by Pagefind for search-result rows).

Single source of truth for per-property metadata is `PROPERTIES` in `src/consts.ts` — `name`, `chipLabel`, `navLabel`, `tagline`, `description`, `channelUrl`, `channelHandle`, `accent`, `accentDark`, `chipText`. (`navLabel` is the short label for the header property button — e.g. `Podcast` instead of the full `The Lonnie & Ryan Podcast`. `chipLabel` is the longer chip text on cards. `name` is the formal display name.)

**To add a new property**, touch all of these in one go:

1. Add the entry in `PROPERTIES` (`src/consts.ts`).
2. Add the slug to `NAV_PROPS` and import the logo into `LOGOS` in **`src/components/Header.astro`** so it appears in the top nav.
3. Add the same logo import to `LOGOS` in **`src/pages/[property].astro`** so the landing page renders it.
4. Drop the channel avatar at `src/assets/brand/<property>-logo.jpg` and a 200×200 search-result version at `public/avatars/<slug>.jpg` (use PIL: center-crop to square, resize, save JPEG quality 85).
5. Set `property: '<slug>'` on relevant posts.

Header (`src/components/Header.astro`) is a two-row sticky bar: row 1 has the four property buttons centered (each = avatar + `navLabel`, tints to `accent` on hover, fills with `accent` when on that property's landing page or one of its posts); row 2 is small text-only links (Home / FAQ / About) plus a Search button. A 4-color accent stripe (the four property accents in equal bands) sits at the bottom edge of the header tying everything together. The same 4-band stripe also crowns the homepage hero card.
...
7. Draft the post in src/content/blog/{slug}.md following the structure above.
   ...
   pubDate (use the YouTube upload date), youtubeId, heroImage, tags, AND
   property (one of 'shed-flips' | 'im-gonna-pack-it' | 'garage-flips' |
   'podcast' — defaults to 'shed-flips' if omitted, but set it explicitly).
...
- **Mixed-feed nickname:** the homepage cross-property feed is labeled "Latest from the Shediverse" (Lonnie's term for the four-property network). Use it when referring to the mixed feed in copy or follow-up UI.

## Generic YouTube CTAs are intentionally absent

As of 2026-05-01 every site-wide "Subscribe on YouTube" / "Watch on YouTube" CTA was removed — header right-cluster YouTube button, post-bottom Watch CTA, property landing-page Subscribe, footer YouTube link, homepage Subscribe button, About-page Subscribe section. Lonnie's call: *"the crazy number of links to videos themselves is good enough."*

**Don't add them back when building new pages or templates.** The engagement path is:

- The video embed at the top of every post (that IS the video).
- Inline timestamp deep-links inside the prose body — `<a href="…?v=ID&t=Ns">[9:18]</a>` pattern, every one opens YouTube at that exact second in a new tab.

These are not "generic" YouTube links — they're surgical. Keep them. Don't bulk in a "Subscribe" CTA at the top, the bottom, the sidebar, or anywhere else.
...
**Where the search UI lives:** `src/components/Header.astro` — the trigger sits in the second nav row (right after About). The overlay and its JS are entirely self-contained in that file — no separate search page or component. Pagefind's default CSS variables are overridden inside `#search { ... }` to match the Shed Flips palette (`--pagefind-ui-primary: var(--accent)`, etc.).

**Per-property search-result avatars.** `BlogPost.astro` sets `data-pagefind-meta="title:…, image[src]:/avatars/<slug>.jpg, image[alt]:<property name>"` on the indexed `<article>`. Pagefind picks up the `image[src]` value and uses it as the result-row thumbnail, so a podcast post in search results shows the Lonnie & Ryan avatar instead of falling back to the first `<img>` on the page (which would always be the Shed Flips logo from the header). The 200×200 JPEGs that back these meta values live at `public/avatars/<slug>.jpg` — see § *Multi-property structure → To add a new property* for the resize/crop convention.
...
- `data/videos.tsv` — every video on the Shed Flips channel
...
**The manifest currently covers Shed Flips only** — the other three properties (I'm Gonna Pack It, Garage Flips, Lonnie & Ryan Podcast) are not yet in `videos.tsv` or `post-candidates.tsv`. For now, the workflow for those properties is "Lonnie names a video, you pull it directly with `yt-dlp` from the channel `/videos` URL" (see § Workflow step 1). Extending the manifest to multi-channel is a future task.
